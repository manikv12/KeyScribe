...
final class FeatureFlagsCrossIDEConversationSharingTests: XCTestCase {
    private func makeIsolatedDefaults() -> UserDefaults {
        let suite = "KeyScribeTests.FeatureFlags.\(UUID().uuidString)"
    ...
    func testCrossIDEConversationSharingResolutionPrefersEnvironmentTrue() {
        ...
        let resolution = FeatureFlags.crossIDEConversationSharingResolution(
            environment: ["KEYSCRIBE_FEATURE_CROSS_IDE_CONVERSATION_SHARING": "true"],
            defaults: defaults
        )
    ...
    func testCrossIDEConversationSharingResolutionPrefersEnvironmentFalse() {
        let defaults = makeIsolatedDefaults()
        defaults.set(true, forKey: "KeyScribe.promptRewriteCrossIDEConversationSharingEnabled")

        let resolution = FeatureFlags.crossIDEConversationSharingResolution(
            environment: ["KEYSCRIBE_FEATURE_CROSS_IDE_CONVERSATION_SHARING": "0"],
            defaults: defaults
        )
