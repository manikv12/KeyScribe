...
final class CodexSessionCatalogTests: XCTestCase {
    ...
    func testLoadSessionsCanFilterToKeyScribeOriginatorOnly() async throws {
        let homeDirectory = try makeTemporaryHomeDirectory()
        defer { try? FileManager.default.removeItem(at: homeDirectory) }

        try writeSession(
            id: "keyscribe-session",
            dayPath: "2026/03/07",
            in: homeDirectory,
            lines: [
                try sessionMetaLine(
        ...
        let sessions = try await catalog.loadSessions(
            ...
            preferredCWD: nil,
            originatorFilter: "KeyScribe"
        )

        XCTAssertEqual(sessions.map(\.id), ["keyscribe-session"])
        ...
        try writeSession(
            ...
            lines: [
                ...
                try responseMessageLine(
                    timestamp: "2026-03-07T10:00:01Z",
                    role: "assistant",
                    text: "This session belongs to KeyScribe."
                )
            ]
        )
    ...
    func testLoadTranscriptFiltersSetupNoiseAndDeduplicatesAgentEchoes() async throws {
        ...
        XCTAssertEqual(transcript.first?.text, "Started a KeyScribe session.")
        ...
        try writeSession(
            ...
            lines: [
                ...
                try functionCallLine(
                    timestamp: "2026-03-07T12:00:03Z",
                    name: "exec_command",
                    arguments: "{\"cmd\":\"rg --files Sources/KeyScribe/Assistant\"}",
                    callID: "call-command"
                ),
                try functionCallOutputLine(
                    timestamp: "2026-03-07T12:00:04Z",
                    callID: "call-command",
                    output: "Output:\nSources/KeyScribe/Assistant/AssistantWindowView.swift"
                ),
                ...
                try customToolCallLine(
                    ...
                    name: "apply_patch",
                    input: "*** Begin Patch\n*** Update File: Sources/KeyScribe/Assistant/AssistantWindowView.swift\n*** End Patch",
                    callID: "call-patch"
                ),
    ...
    private func makeTemporaryHomeDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("KeyScribeCodexCatalogTests-\(UUID().uuidString)", isDirectory: true)
