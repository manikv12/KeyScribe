...
final class AssistantMemoryRetrievalServiceTests: XCTestCase {
    ...
    func testPrepareTurnContextIncludesMemoryStorageGuideForMemorySavePrompts() throws {
        let memoryRoot = try makeTemporaryDirectory(named: "assistant-memory")
        let databaseURL = try makeDatabaseURL()
        let store = try MemorySQLiteStore(databaseURL: databaseURL)
        let threadMemoryService = AssistantThreadMemoryService(baseDirectoryURL: memoryRoot)
        ...
        let built = try retrievalService.prepareTurnContext(
            threadID: "thread-memory-guide",
            prompt: "Save it in memory and tell me where the short term and long term memory are stored",
            cwd: "/tmp/OpenAssist",
            summaryMaxChars: 2200
        )

        let summary = try XCTUnwrap(built.summary)
        let expectedThreadMemoryPath = memoryRoot
            .appendingPathComponent("thread-memory-guide", isDirectory: true)
            .appendingPathComponent("memory.md", isDirectory: false)
            .path

        XCTAssertTrue(summary.contains("Memory storage guide:"))
        XCTAssertTrue(summary.contains(expectedThreadMemoryPath))
        XCTAssertTrue(summary.contains(databaseURL.path))
        XCTAssertTrue(summary.contains("assistant_memory_entries"))
        XCTAssertTrue(summary.contains("conversation_agent_profiles"))
        XCTAssertTrue(summary.contains("AGENTS.md"))
    }

    func testPrepareTurnContextIncludesMemoryStorageGuideForSaveItInTheMemoryPrompt() throws {
        let memoryRoot = try makeTemporaryDirectory(named: "assistant-memory")
        let databaseURL = try makeDatabaseURL()
        let store = try MemorySQLiteStore(databaseURL: databaseURL)
        let threadMemoryService = AssistantThreadMemoryService(baseDirectoryURL: memoryRoot)
        ...
        let built = try retrievalService.prepareTurnContext(
            threadID: "thread-memory-variation",
            prompt: "If I ask the agent to save it in the memory, it should know where the short term and long term memory are",
            cwd: "/tmp/OpenAssist",
            summaryMaxChars: 2200
        )
