...
actor AssistantComputerUseService {
    ...
    private enum ServiceError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .accessibilityRequired:
                return "Computer Use needs macOS Accessibility permission so KeyScribe can click and type."
            case .screenRecordingRequired:
                return "Computer Use needs macOS Screen Recording permission so KeyScribe can see the current screen."
            case .invalidArguments:
                ...
                if let computerCall = Self.extractComputerCall(from: response.raw) {
                    if !computerCall.pendingSafetyChecks.isEmpty {
                        ...
                        throw ServiceError.unsupportedSafetyCheck(
                            joined.isEmpty
                                ? "Computer Use paused because OpenAI returned a safety check that this version of KeyScribe cannot confirm yet."
    ...
    private func resolveAuth(preferredModelID: String?) async throws -> AuthContext {
        ...
        let explicitModel = ProcessInfo.processInfo.environment["KEYSCRIBE_COMPUTER_USE_MODEL"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
