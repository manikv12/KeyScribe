...
actor LocalAIRuntimeManager: LocalAIRuntimeManaging {
    ...
    func installManagedRuntime() async throws -> LocalAIRuntimeDetection {
        ...
        let tempZipURL = fileManager.temporaryDirectory.appendingPathComponent("keyscribe-local-ai-\(UUID().uuidString).zip")
        let extractionRoot = fileManager.temporaryDirectory.appendingPathComponent("keyscribe-local-ai-runtime-\(UUID().uuidString)")
