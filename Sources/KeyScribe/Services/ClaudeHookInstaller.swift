...
enum ClaudeHookInstallOption: String, CaseIterable, Hashable, Identifiable {
    ...
    var detailText: String {
        switch self {
        case .notification:
            return "Optional. Claude only supports this event through its command format. KeyScribe adds that part for you automatically."
        case .stop:
...
enum ClaudeHookInstallerError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .missingToken:
            return "KeyScribe could not find the Automation API token."
        case .invalidSettingsFormat:
    ...
    func installSelectedHooks(
        ...
        if fileManager.fileExists(atPath: settingsURL.path) {
            backupURL = settingsURL
                ...
                .appendingPathComponent("settings.json.keyscribe-backup-\(Self.backupTimestamp())")
