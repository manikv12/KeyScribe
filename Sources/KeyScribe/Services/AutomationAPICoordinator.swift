...
final class AutomationAPICoordinator: NSObject, ObservableObject {
    ...
    private static let automationNotificationIdentifierPrefix = "keyscribe-automation-"
    private static let automationNotificationUserInfoKey = "keyscribeAutomation"
    ...
    func requestNotificationPermissionIfNeeded() async {
        ...
        if notificationAuthorizationState == .denied {
            serverMessage = "Enable notifications for KeyScribe in macOS Notification settings."
            return
        }
        ...
        do {
            let response = try await deliver(
                AutomationAPIAnnounceRequest(
                    message: "This is a KeyScribe automation test.",
                    title: "KeyScribe",
                    subtitle: "Automation API",
                    channels: channels,
                    voiceIdentifier: nil,
                    sound: nil,
                    source: "keyscribe.settings-test",
                    dedupeKey: nil,
    ...
    func codexCLINotifyExample(for settings: SettingsStore) -> String {
        ...
        let token = settings.automationAPIToken.trimmingCharacters(in: .whitespacesAndNewlines)
        return """
        notify = ["sh", "-lc", "curl -sS -X POST http://127.0.0.1:\(port)/automation/v1/hooks/codex -H 'Content-Type: application/json' -H 'Authorization: Bearer \(token)' --data-binary \\\"$1\\\"", "keyscribe-codex"]
        """
    }
    ...
    private func resolvedAnnouncement(from request: AutomationAPIAnnounceRequest) throws -> AutomationAPIResolvedAnnouncement {
        ...
        let title = collapseWhitespace(request.title).isEmpty ? "KeyScribe" : collapseWhitespace(request.title)
