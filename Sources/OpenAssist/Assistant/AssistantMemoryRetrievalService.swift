...
final class AssistantMemoryRetrievalService {
    ...
    func prepareTurnContext(
        ...
        let summary = buildSummary(
            ...
            prompt: prompt,
            threadMemoryFileURL: resolvedChange.fileURL,
    ...
    private func buildSummary(
        ...
        if shouldIncludeMemoryStorageGuide(for: prompt) {
            sections.append(memoryStorageGuide(
                threadMemoryFileURL: threadMemoryFileURL,
    ...
    private func shouldIncludeMemoryStorageGuide(for prompt: String) -> Bool {
        ...
        if triggerPhrases.contains(where: { normalized.contains($0) }) {
            return true
        }

        let hasSaveOrRemember = normalized.contains("save") || normalized.contains("remember") || normalized.contains("store")
        let hasMemory = normalized.contains("memory")
        let hasLocationIntent = normalized.contains("where")
        ...
        return hasMemory && (hasSaveOrRemember || hasLocationIntent)
    }

    private func memoryStorageGuide(
        threadMemoryFileURL: URL,
        databaseURL: URL
    ) -> String {
        """
        Memory storage guide:
        - Short-term session memory for this chat is the markdown file at \(threadMemoryFileURL.path)
        - Long-term reusable assistant memory is stored in the SQLite database at \(databaseURL.path), mainly in the `assistant_memory_entries` table
        - Thread-specific agent state is also stored in that same database in the `conversation_agent_profiles`, `conversation_agent_entities`, and `conversation_agent_preferences` tables
        - Do not confuse these with local note files like `AGENTS.md`; those are not OpenAssist long-term assistant memory
        """
    }
